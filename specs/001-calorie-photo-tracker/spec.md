...
- **FR-003b**: Если email-сервис недоступен при отправке письма подтверждения, система ДОЛЖНА использовать retry механизм с экспоненциальной задержкой: 3 попытки с интервалами 1 минута, 5 минут, 15 минут. Если все попытки неуспешны, пользователь видит сообщение "Не удалось отправить письмо подтверждения" с возможностью повторной отправки через профиль
...
- **FR-006**: Система ДОЛЖНА распознавать продукты питания на загруженных изображениях с точностью не менее 75% для распространенных продуктов (Top-1 accuracy: правильная идентификация продукта в первом результате распознавания, см. SC-004). **Распространенные продукты** определяются как ~500 наиболее часто фотографируемых продуктов питания. Список включает:
  - Базовые злаки: рис (белый, коричневый), гречка, макароны, овсянка, перловка
  - Мясо: курица (грудка, бедро), говядина, свинина, индейка, баранина
  - Овощи: помидоры, огурцы, капуста (белокочанная, цветная), картофель, морковь, лук, перец, баклажаны, кабачки
  - Фрукты: яблоки, бананы, апельсины, груши, виноград, арбуз, дыня, киви
  - Молочные продукты: молоко, сыр (твердый, мягкий), йогурт, творог, сметана, кефир
  - Яйца: куриные яйца (вареные, жареные)
  - Хлеб и выпечка: белый хлеб, черный хлеб, булочки, батон
  - Распространенные готовые блюда: борщ, суп, салат оливье, котлеты, пельмени, плов
  - **Источник данных**: USDA FoodData Central API + локальная seed база с русскими/международными продуктами (см. prisma/seed-data/common-foods.json). Vision model (OpenRouter Claude) обучен на широком спектре продуктов, поэтому 75% accuracy достижима для этого набора
...
- **FR-022a**: Пользователи ДОЛЖНЫ иметь возможность редактировать вес и БЖУ вручную добавленных продуктов в несохраненном приеме пищи (до момента сохранения в профиль)
- **FR-022b**: Пользователи ДОЛЖНЫ иметь возможность редактировать или удалять продукты из сохраненных приемов пищи через интерфейс редактирования meal (доступ через calendar day details или profile history)
